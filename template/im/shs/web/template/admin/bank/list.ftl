...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>银行列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as bank]
				    <tr>
				        <td>
				        	<a href="${baseUrl}/bank_branch?bankId=${bank.id}" class="hint--top hint--info" data-hint="查看银行支行">${bank.name}</a>
				        </td>
				        <td>${bank.code}</td>
				        <td>${bank.payable?string("是", "否")}</td>
				        <td>${bank.order}</td>
				        <td>${bank.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				        <td class="center-left">
							<a href="${indexUrl}/${bank.id}/edit">
								<i class="icon-pencil"></i>
								编辑
							</a>
							<br/>
							<a href="${indexUrl}/delete" class="deleteRow" deleteId="${bank.id}">
