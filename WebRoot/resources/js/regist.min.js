...
$().ready(function() {
	...
	rsaKey.setPublic(b64tohex(modulus), b64tohex(exponent));
		...
		rules: {
			mobile: {
				...
				pattern: /^1[3,5,8]\d{9}$/,
				remote: {
					url: "/P2P-2.0-YuXinChuangTou/regist/check_mobile",
					type: "post",
					cache: false
				}
			},
			username: {
				...
				maxlength: 20,
				remote: {
					url: "/P2P-2.0-YuXinChuangTou/regist/check_username",
					type: "post",
					cache: false
				}
			},
		...
		messages: {
			...
			username: {
				required: "请输入2-20位用户名",
				pattern: "用户名只允许包含中文、英文、数字、下划线",
				minlength: "请输入2-20位用户名",
				maxlength: "请输入2-20位用户名",
				remote: "用户名已存在"
			},
			password: {
				required: "请输入4-20位密码",
				pattern: "密码包含非法字符",
				minlength: "请输入2-20位密码",
				maxlength: "请输入2-20位密码"
			},
			rePassword: {
				required: "请输入确认密码",
				equalTo: "两次密码输入不一致"
		...
		submitHandler: function(form) {
			...
			$.ajax({
				url: "/P2P-2.0-YuXinChuangTou/regist/texting_regist",
				data: {
					mobile: $mobile.val(),
	...
	$captchaButton.click(function(e) {
		...
		if(!$validate.element($mobile) || !$validate.element($username)) {
			return false;
		}
		
		$.ajax({
			url: "/P2P-2.0-YuXinChuangTou/regist/texting_apply",
			data: {
				mobile: $mobile.val(),
				username: $username.val()
			},
