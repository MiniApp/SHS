...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>银行支行列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as bankBranch]
				    <tr>
						<td class="center tableCheckbox">
							<input class="selectRow" type="checkbox" name="ids" value="${bankBranch.id}" />
						</td>
				        <td>${bankBranch.name}</td>
				        <td>${bankBranch.locality}</td>
				        [#if !bank??]
				        	<td>${bankBranch.bank}</td>
				        [/#if]
				        <td>${bankBranch.order}</td>
				        <td>${bankBranch.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				        <td class="center">
							<a href="${indexUrl}/${bankBranch.id}/edit">
		...
		<div class="hide">
		    ...
		    <div class="dataTableActions">
		    	[#if bank??]
					<a href="${baseUrl}/bank" class="btn btn-default btn-sm">
						<i class="icon-mail-reply-all"></i>
						返回银行
					</a>
				[/#if]
		        <div class="btn-group">
					<a href="${indexUrl}/new[#if bank??]?bankId=${bank.id}[/#if]" class="btn btn-default btn-sm">
						<i class="icon-plus"></i>
						添加
					</a>
					<a href="${indexUrl}[#if bank??]?bankId=${bank.id}[/#if]" class="btn btn-default btn-sm">
