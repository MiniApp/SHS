...
ObjectUtil
		.define(
				"crm.pages.ocrm.common.systemManage.productLibrary.productMeal.ProductMealEdit",
				"base.PageObject",
				{
					htmlUrl : Constants.CONTEXT_PATH
							+ "/pages/ocrm/common/systemManage/productLibrary/productMeal/ProductMealEdit.html",// 页面url地址
					initData : function() {
						...
						ConnectionUtil.ajaxReq({
									...
									callback : function(data) {
										...
										DataUtil
												.populateDataForArea(
														data,
														owner.ids.productMealAdd);// 渲染数据到页面
									}
								});
							...
							buttons : [
							           {
											...
											hidden : owner.flag,
											handler : function() {
							           			if(DataUtil.isEmpty(owner.tbproproductgroupdetailky)){
					...
					saveProductMeal : function() {
						...
						if (data != Constants.VALIDATION_FAIL) {// 如果data不等Constants.VALIDATION_FAIL则表示验证通过
							...
							ConnectionUtil.ajaxReq({// 发送ajax请求
								strServId : "productMealService.updateProductMeal",
								jsonData : newData,
								callback : function(data) {
