...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>转账提现[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
		...
		<div class="form-horizontal">
			...
			<div class="tabbable">
				...
				<ul class="nav nav-tabs">
					...
					[/#if]
					<li>
						<a id="transfer_tab" href="#transfer" data-toggle="tab">提现转账</a>
					</li>
				</ul>
				...
				<div class="tab-content">
					...
					[#-- 提现转账 --]
					<div id="transfer" class="tab-pane" tab-id="transfer_tab">
				    	[#-- 表单 --]
						<form id="inputForm" action="${indexUrl}/${id}" method="post">
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<select id="approved" class="form-control chosen-select" name="approved" data-placeholder="&nbsp;">
										[#if bankCard != null && bankCard.state == "success" && bankCard.verifyCardholder(pers.id)]
							...
							<div class="form-group collapse in">
								<label for="transferTime" class="col-sm-2 control-label">
									<span class="required">*</span>
									转账时间
								</label>
								<div class="col-sm-4">
									<input id="transferTime" class="form-control datetimepicker datetimepicker-yyyy-MM-dd-HH-mm" type="text" name="transferTime" />
								</div>
							</div>
	...
	[#-- withdrawal.transfer 提现转账 --]
	<script type="text/javascript" src="${base}/resources/admin/js/withdrawal.transfer.min.js"></script>
    [#-- 有瞬时消息时 --]
	[#if flashMessage != null]
	    [#-- HubSpot Messenger 弹框（Alert）组件库 --]
