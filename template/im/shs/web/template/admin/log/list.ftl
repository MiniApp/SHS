...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>日志列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
				...
				[#list list as log]
				    <tr>
						<td class="center tableCheckbox">
							<input class="selectRow" type="checkbox" name="ids" value="${log.id}" />
						</td>
				        <td>${log.operation}</td>
				        <td>${log.operator!"-"}</td>
				        <td>${log.ip}</td>
		...
		<div class="hide">
		    ...
		    <div class="dataTableActions">
				...
				<a href="${indexUrl}/clear" class="btn btn-default btn-sm clearRows">
		    ...
		    [#-- clear confirmation box 清空确认对话框 --]
		    <div class="colorbox-content clearConfirmDialog">
		        <div class="colorbox-message">
		        	<strong>确定清空所有记录吗？</strong>
		    	</div>
		        <div class="colorbox-button">
		            <a href="#" class="btn btn-primary btn-xs confirmYes">&nbsp;确&nbsp;定&nbsp;</a>
		            &nbsp;&nbsp;
