...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>待还款的还款记录[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as repaymentRecord]
					<tr>
						<td class="center tableCheckbox">
							<input class="selectRow" type="checkbox" name="ids" value="${repaymentRecord.id}" />
						</td>
						[#if !repayment??]
							<td class="center">${repaymentRecord.repayment.id}</td>
						[/#if]
						[#-- 还款记录 List TD --]
						[#include "/template/admin/repayment_record/list_tds.ftl" /]
				        <td class="center">
				        	<a href="${indexUrl}/${repaymentRecord.id}?[#if repayment?? && borrowing??]repayment=${repayment}[#elseif borrowing??]borrowing=${borrowing}[/#if]">
								<i class="icon-jpy"></i>
								还款
							</a>
				        </td>
		...
		<div class="hide">
		    ...
		    <div class="dataTableActions">
		    	[#if repayment?? && borrowing??]
					<a href="${baseUrl}/borrowing_repayment?borrowing=${borrowing}" class="btn btn-default btn-sm">
    ...
    [#-- borrowing.repayment_record.list 借款还款记录列表 --]
	<script type="text/javascript" src="${base}/resources/admin/js/borrowing.repayment_record.list.min.js"></script>
	[#-- Colorbox 对话框 --]
