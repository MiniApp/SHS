...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>添加文章[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<form id="inputForm" class="form-horizontal" action="${indexUrl}/create" method="post">
			[#if articleCategory??]
				<input type="hidden" name="articleCategoryId" value="${articleCategory.id}" />
			[/#if]
				...
				<div class="tab-content">
					...
					<div id="basic" class="tab-pane active" tab-id="basic_tab">
						...
						<div class="form-group">
							...
							<input id="categoryId" class="selectSascade articleCategory" type="hidden" name="categoryId" value="${(articleCategory.id)!}" treePath="${(articleCategory.treePath)!}" />
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<input id="author" class="form-control" type="text" name="author" maxlength="20" />
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<textarea id="cont" class="form-control kind-editor" name="cont" cols="10" rows="5"></textarea>
							</div>
						</div>
			...
			<div class="form-group">
				<div class="col-sm-offset-2 col-sm-4">
					...
					<button class="btn btn-link btn-sm" type="button" onclick="location.href='${indexUrl}[#if articleCategory??]?articleCategoryId=${articleCategory.id}[/#if]'">&nbsp;返&nbsp;回&nbsp;</button>
				</div>
			</div>
	...
	[#-- article.add 文章添加 --]
	<script type="text/javascript" src="${base}/resources/admin/js/article.add.min.js"></script>
    [#-- 有瞬时消息时 --]
	[#if flashMessage != null]
	    [#-- HubSpot Messenger 弹框（Alert）组件库 --]
