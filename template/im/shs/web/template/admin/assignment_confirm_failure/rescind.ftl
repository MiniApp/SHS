...
[#-- 索引URL --]
[#assign indexUrl = baseUrl + "/assignment_confirm_failure" /]
<!DOCTYPE html>
<html lang="en">
	<head>
		[#-- meta 标签 --]
		...
		[#-- 标题 --]
		<title>撤销转让确认[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    [#-- assignment.confirm.rescind 转让确认撤销 --]
	<script type="text/javascript" src="${base}/resources/admin/js/assignment.confirm.rescind.min.js"></script>
    [#-- 有瞬时消息时 --]
	[#if flashMessage != null]
	    [#-- HubSpot Messenger 弹框（Alert）组件库 --]
