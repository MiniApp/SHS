...
[#-- 索引URL --]
[#assign indexUrl = baseUrl + "/assignment_buyback" /]
<!DOCTYPE html>
<html lang="en">
	<head>
		[#-- meta 标签 --]
		...
		[#-- 标题 --]
		<title>待回购的转让列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
				...
				[#list list as assignment]
				    <tr>
				        ...
				        <td class="center-left">
							...
							<br/>
				        	<a href="${baseUrl}/buyback?assignment=${assignment.id}">
								<i class="icon-eye-open"></i>
								查看回购
							</a>
							<br/>
				        	<a href="${baseUrl}/buyback_plan?assignment=${assignment.id}">
								<i class="icon-eye-open"></i>
								查看回购计划
							</a>
				        </td>
