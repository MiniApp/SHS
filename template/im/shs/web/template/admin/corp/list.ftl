...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>公司列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
			...
			<tbody>
				[#list list as corp]
				    <tr>
						...
						<td>${corp.corpName!"-"}</td>
				        <td>${corp.corpType!"-"}</td>
				        <td>${corp.corpDomain!"-"}</td>
				        <td>${corp.corpScale!"-"}</td>
				        <td>${corp.corpWithGuarantee?string("有", "无")}</td>
				        <td>
				        	${corp.name!"-"}
				        	[#if corp.idNo??]
				        		<br/>
				        		<small>${corp.idNo}</small>
				        	[/#if]
				        </td>
				        <td>${corp.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				        <td class="center">
							<a href="${indexUrl}/${corp.id}/modify">
								<i class="icon-pencil"></i>
								修改
							</a>
							/
							<a href="${indexUrl}/${corp.id}/logout">
								<i class="icon-plus"></i>
								注销
							</a>
			        	</td>
