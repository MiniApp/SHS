...
public class BasicSettingController extends BaseAdminController {
    ...
    public String setting(ModelMap model) {
        ...
        return "/im/shs/web/setting/basic/setting";
    }
    ...
    public String setting(BasicSetting basicSetting, RedirectAttributes redirectAttributes) {
        ...
        // 选择网站LOGO图片文件时
        if (basicSetting.getSiteLogoFile() != null && !basicSetting.getSiteLogoFile().isEmpty()) {

            // 验证网站LOGO图片文件类型
            if (!fileService.verify(FileTypeEnum.image, basicSetting.getSiteLogoFile())) {
                addFlashMessage(redirectAttributes, ERROR_MESSAGE);
                return INDEX_REDIRECT_URL;
            }

            // 上传网站LOGO图片文件
            String siteLogoUploadPath = fileService.uploadLocal(FileTypeEnum.image, basicSetting.getSiteLogoFile());
