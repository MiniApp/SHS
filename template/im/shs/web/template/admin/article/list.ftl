...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>文章列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
				...
				[#list list as article]
				    <tr>
						<td class="center tableCheckbox">
							<input class="selectRow" type="checkbox" name="ids" value="${article.id}" />
						</td>
				        <td>${article.title}</td>
		        		[#if !articleCategory??]
				        	<td>${article.category}</td>
				        [/#if]
				        <td>${article.published?string("是", "否")}</td>
				        <td>${article.top?string("是", "否")}</td>
				        <td>${article.order}</td>
				        <td>${article.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				        <td class="center">
							<a href="${indexUrl}/${article.id}/edit[#if articleCategory??]?articleCategoryId=${articleCategory.id}[/#if]">
		...
		<div class="hide">
		    ...
		    <div class="dataTableActions">
		    	[#if articleCategory??]
					<a href="${baseUrl}/article_category[#if articleCategory.parent??]?parentId=${articleCategory.parent.id}[/#if]" class="btn btn-default btn-sm">
						<i class="icon-mail-reply-all"></i>
						返回文章分类
					</a>
				[/#if]
		        <div class="btn-group">
					<a href="${indexUrl}/new[#if articleCategory??]?articleCategoryId=${articleCategory.id}[/#if]" class="btn btn-default btn-sm">
						<i class="icon-plus"></i>
						添加
					</a>
					<a href="${indexUrl}[#if articleCategory??]?articleCategoryId=${articleCategory.id}[/#if]" class="btn btn-default btn-sm">
