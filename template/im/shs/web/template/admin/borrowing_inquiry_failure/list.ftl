...
[#-- 索引URL --]
[#assign indexUrl = baseUrl + "/borrowing_inquiry_failure" /]
<!DOCTYPE html>
<html lang="en">
	<head>
		[#-- meta 标签 --]
		...
		[#-- 标题 --]
		<title>调查已失败的借款列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<table class="table table-striped table-bordered dataTable">
			<thead>
				<tr>
					[#-- 借款 List TH --]
					[#include "/template/admin/borrowing/list_ths.ftl" /]
			        <th class="center">操作</th>
				</tr>
			</thead>
			<tbody>
				[#list list as borrowing]
				    <tr>
						[#-- 借款 List TD --]
						[#include "/template/admin/borrowing/list_tds.ftl" /]
				        <td class="center">
				        	<a href="${indexUrl}/${borrowing.id}">
								<i class="icon-undo"></i>
								撤销
							</a>
				        </td>
