...
ObjectUtil
		.define(
				"crm.pages.ocrm.common.systemManage.productLibrary.product.update.ProductInvestInfoUpdate",
				"base.PageObject",
				{
					htmlUrl : Constants.CONTEXT_PATH
							+ "/pages/ocrm/common/systemManage/productLibrary/product/update/ProductInvestInfoUpdate.html",// 页面url地址
	...
	initData : function() {
		               ...
		               var owner = this;						
						...
						ConnectionUtil.ajaxReq( {
							strServId : "productManageService.getInvestInfoById",
	...
	initCmp : function() {
        ...
        //开始时间
								ObjectUtil.create('component.DateField', {
									renderTo : owner.ids.startdateStr,
									format : 'Y-m-d'
								});
		//结束时间
								ObjectUtil.create('component.DateField', {
									renderTo : owner.ids.finishdateStr,
									format : 'Y-m-d'
								});
								//起售日期
								ObjectUtil.create('component.DateField', {
									renderTo : owner.ids.stadateStr,
									format : 'Y-m-d'
								});
								//截止日期
								ObjectUtil.create('component.DateField', {
									renderTo : owner.ids.enddateStr,
									format : 'Y-m-d'
								});
        // 面板
	...
	updateIdentify : function(){
		...
		if (data != Constants.VALIDATION_FAIL) {// 如果data不等Constants.VALIDATION_FAIL则表示验证通过
			...
			ConnectionUtil.ajaxReq({// 发送ajax请求
				strServId : "productManageService.updateInvestInfo",
				jsonData : newData,
				callback : function(msg) {
					MsgUtil.alert("提示", "投资产品信息修改成功!");
