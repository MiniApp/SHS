...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>设置阿里云存储[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<form id="inputForm" class="form-horizontal" action="${indexUrl}/${plugin.settingUrl}" method="post">
			...
			[#-- ACCESS ID --]
			<div class="form-group">
				<label for="accessId" class="col-sm-2 control-label">
					<span class="required">*</span>
					ACCESS ID
				</label>
				<div class="col-sm-4">
					<input id="accessId" class="form-control" type="text" name="accessId" value="${plugin.attributes['accessId']}" maxlength="200" />
				</div>
			</div>
			
			[#-- ACCESS KEY --]
			<div class="form-group">
				<label for="accessKey" class="col-sm-2 control-label">
					<span class="required">*</span>
					ACCESS KEY
				</label>
				<div class="col-sm-4">
					<input id="accessKey" class="form-control" type="text" name="accessKey" value="${plugin.attributes['accessKey']}" maxlength="200" />
				</div>
			</div>
			
			[#-- BUCKET名称 --]
			<div class="form-group">
				<label for="bucketName" class="col-sm-2 control-label">
					<span class="required">*</span>
					BUCKET名称
				</label>
				<div class="col-sm-4">
					<input id="bucketName" class="form-control" type="text" name="bucketName" value="${plugin.attributes['bucketName']}" maxlength="200" />
				</div>
			</div>
	...
	[#-- storage_plugin.oss.setting 阿里云存储设置 --]
	<script type="text/javascript" src="${base}/resources/admin/js/storage_plugin.oss.setting.min.js"></script>
    [#-- 有瞬时消息时 --]
	[#if flashMessage != null]
	    [#-- HubSpot Messenger 弹框（Alert）组件库 --]
