...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>编辑广告[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		[#-- 表单 --]
		<form id="inputForm" class="form-horizontal" action="${indexUrl}/${ad.id}" method="post" enctype="multipart/form-data">
			[#if adPositionId??]
				...
				<div class="col-sm-4">
					<select id="position" class="form-control chosen-select" name="positionId" data-placeholder="&nbsp;">
						[#list adPositions as adPosition]
							<option value="${adPosition.id}"[#if adPosition == ad.position] selected="selected"[/#if]>${adPosition.name} [${adPosition.width} × ${adPosition.height}]</option>
						[/#list]
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<select id="type" class="form-control chosen-select" name="type" data-placeholder="&nbsp;">
						[#list types as type]
							<option value="${type}"[#if type == ad.type] selected="selected"[/#if]>${message("AdType." + type)}</option>
						[/#list]
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input id="title" class="form-control" type="text" name="title" value="${ad.title}" maxlength="200" />
				</div>
			</div>
				...
				<div class="form-group collapse in">
					...
					<div class="col-sm-4">
						<div class="input-group input-file">
							<input class="form-control file-name" value="${base}${ad.path}" />
							<span class="input-group-btn">
								<span class="btn btn-default btn-file">
									修改图片
									<input id="image" class="ignore" type="file" name="imageFile" />
								</span>
								<a href="${base}${ad.path}" class="btn btn-default" target="_blank">查看图片</a>
							</span>
						</div>
					</div>
				</div>
					...
					<div class="col-sm-4">
						<div class="input-group input-file">
							...
							<span class="input-group-btn">
								...
								<a href="${base}${ad.path}" class="btn btn-default" target="_blank">查看Flash</a>
							</span>
						</div>
					</div>
				</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input id="url" class="form-control" type="text" name="url" value="${ad.url}" maxlength="200" />
				</div>
			</div>
			...
			[#if ad.type == "text"]
				<div class="form-group collapse in">
					...
					<div class="col-sm-8">
						<textarea id="cont" class="form-control kind-editor" name="cont" cols="10" rows="5" style="width: 100%;">${ad.cont}</textarea>
					</div>
				</div>
			[#else]
				<div class="form-group collapse">
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<textarea id="cont" class="form-control" name="cont" cols="10" rows="5">${(ad.cont?html)!}</textarea>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input id="startDate" class="form-control datetimepicker datetimepicker-yyyy-MM-dd" type="text" name="startDate" value="${(ad.startDate?string("yyyy-MM-dd"))!}" />
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input id="endDate" class="form-control datetimepicker datetimepicker-yyyy-MM-dd" type="text" name="endDate" value="${(ad.endDate?string("yyyy-MM-dd"))!}" />
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<input id="order" class="form-control" type="text" name="order" value="${ad.order}" maxlength="9" />
				</div>
			</div>
	...
	[#-- ad.edit 广告编辑 --]
	<script type="text/javascript" src="${base}/resources/admin/js/ad.edit.min.js"></script>
    [#-- 有瞬时消息时 --]
	[#if flashMessage != null]
	    [#-- HubSpot Messenger 弹框（Alert）组件库 --]
