Ext.define('ux.date.DateTimePicker', {
				...
				if (this.showTime) {
					if (!this.timefield) {
						this.timefield = Ext.create(
								'ux.date.TimePickerField', {
									fieldLabel : this.timeLabel,
									labelWidth : 40,
									value : Ext.Date
											.format(this.value, 'H:i:s')
								});
					}
					this.timefield.ownerCt = this;
					this.timefield.on('change', this.timeChange, this);
					this.callParent(arguments);
					var tfEl = Ext.core.DomHelper.insertBefore(Ext.get(this.el.dom.id+"-footerEl"), {
								tag : 'div',
								style : 'border:10px;',
								children : [{
											tag : 'div',
					...
					this.timefield
							.render(this.el.child('div div.ux-timefield'));
