...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>账户列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as account]
				    <tr>
				        <td>${account.id}</td>
				        <td>${account.credit?string("currency")}</td>
				        <td>${account.debit?string("currency")}</td>
				        <td>${account.frozen?string("currency")}</td>
				        <td>${account.balance?string("currency")}</td>
				        <td>${account.available?string("currency")}</td>
				        <td>
				        	${account}
							[#if account.name??]
								<br/>
							...
							[#if account.idNo??]
								<br/>
								<small>${account.idNo}</small>
							[/#if]
				        </td>
				        <td>${account.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				        <td class="center-left">
							<a href="${indexUrl}/${account.id}">
								<i class="icon-eye-open"></i>
								查看
							</a>
							<br/>
							<a href="${baseUrl}/account_recharge/${account.id}?redirectType=list">
								<i class="icon-jpy"></i>
								充值
							</a>
							<br/>
							<a href="${baseUrl}/account_charge/${account.id}?redirectType=list">
								<i class="icon-jpy"></i>
								扣费
							</a>
			        	</td>
