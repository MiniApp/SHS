...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>设置宝付支付[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<form id="inputForm" class="form-horizontal" action="${indexUrl}/${plugin.settingUrl}" method="post" enctype="multipart/form-data">
			...
			[#-- 商户终端号 --]
			<div class="form-group">
				<label for="terminal" class="col-sm-2 control-label">
					<span class="required">*</span>
					商户终端号
				</label>
				<div class="col-sm-4">
					<input id="terminal" class="form-control" type="text" name="terminal" value="${plugin.terminal}" maxlength="200" />
				</div>
			</div>
	...
	[#-- payment_plugin.baofoo.setting 宝付支付设置 --]
	<script type="text/javascript" src="${base}/resources/admin/js/payment_plugin.baofoo.setting.min.js"></script>
    [#-- 有瞬时消息时 --]
	[#if flashMessage != null]
	    [#-- HubSpot Messenger 弹框（Alert）组件库 --]
