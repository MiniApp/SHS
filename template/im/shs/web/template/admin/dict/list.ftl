...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>词典列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as dict]
				    <tr>
				        <td>
				        	<a href="${baseUrl}/dict_word?dictId=${dict.id}" class="hint--top hint--info" data-hint="查看单词">${dict.name}</a>
			        	</td>
			        	<td>${dict.ident}</td>
				        <td>${dict.builtin?string("是", "否")}</td>
				        <td>
							[#if dict.description??]
								${abbreviate(dict.description, 50, "...")}
							[#else]
								-
							[/#if]
				        </td>
				        ...
				        <td class="center-left">
							<a href="${indexUrl}/${dict.id}/edit">
								<i class="icon-pencil"></i>
								编辑
							</a>
							<br/>
							<a href="${indexUrl}/delete" class="deleteRow" deleteId="${dict.id}">
