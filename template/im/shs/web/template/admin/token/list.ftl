...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>令牌列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as token]
				    <tr>
				        <td>${message("RandomStringType." + token.type)}</td>
				        <td>${message("TokenMethod." + token.method)}</td>
				        <td>${token.addr}</td>
				        <td>${token.code}</td>
				        <td>${(token.expiry?string("yyyy-MM-dd HH:mm:ss"))!"-"}</td>
				        <td>${(token.retry?string("yyyy-MM-dd HH:mm:ss"))!"-"}</td>
				        <td>${token.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				    </tr>
				[/#list]
			</tbody>
		</table>
