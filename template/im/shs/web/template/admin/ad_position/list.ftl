...
<html lang="en">
    ...
    <body class="contentwrapper">
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as adPosition]
				    <tr>
				        <td>${adPosition.name}</td>
			        	<td>${adPosition.ident}</td>
				        <td>${adPosition.builtin?string("是", "否")}</td>
				        <td>${adPosition.width}</td>
				        <td>${adPosition.height}</td>
				        <td>
							[#if adPosition.description??]
								${abbreviate(adPosition.description, 50, "...")}
							[#else]
								-
							[/#if]
						</td>
				        <td class="center-left">
							<a href="${indexUrl}/${adPosition.id}/edit">
								<i class="icon-pencil"></i>
								编辑
							</a>
							<br/>
							<a href="${indexUrl}/delete" class="deleteRow" deleteId="${adPosition.id}">
								<i class="icon-remove"></i>
								删除
							</a>
							<br/>
							<a href="${baseUrl}/ad?adPositionId=${adPosition.id}">
