...
/**
 * @class : ImageUtils
 * @description: 图片处理(支持JDK、GraphicsMagick、ImageMagick)
 *
 * @author suhao
 * @date 2014年7月13日 上午1:28:27
 * @version 1.0
 */
public final class ImageUtils {
    ...
    public static void addWatermark(File srcFile, File destFile, File watermarkFile,
            WatermarkPositionEnum watermarkPosition, int alpha) {
        ...
        if (watermarkFile == null || !watermarkFile.exists() || watermarkPosition == null
                || watermarkPosition == WatermarkPositionEnum.no) {
            try {
        ...
        if (type == Type.jdk) {
            ...
            try {
                ...
                if (watermarkPosition == WatermarkPositionEnum.topLeft) {
                    x = 0;
                    y = 0;
                } else if (watermarkPosition == WatermarkPositionEnum.topRight) {
                    x = srcWidth - watermarkImageWidth;
                    y = 0;
                } else if (watermarkPosition == WatermarkPositionEnum.center) {
                    x = (srcWidth - watermarkImageWidth) / 2;
                    y = (srcHeight - watermarkImageHeight) / 2;
                } else if (watermarkPosition == WatermarkPositionEnum.bottomLeft) {
                    x = 0;
                    y = srcHeight - watermarkImageHeight;
                } else if (watermarkPosition == WatermarkPositionEnum.bottomRight) {
                    x = srcWidth - watermarkImageWidth;
                    y = srcHeight - watermarkImageHeight;
                }
    ...
    private static String toHexEncoding(Color color) {
        ...
        R = StringUtils.length(R) == 1 ? "0" + R : R;
        G = StringUtils.length(G) == 1 ? "0" + G : G;
        B = StringUtils.length(B) == 1 ? "0" + B : B;
        stringBuffer.append("#");
