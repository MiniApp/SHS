...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>词典单词列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
    ...
    <body class="contentwrapper">
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as dictWord]
				    <tr>
						<td class="center tableCheckbox">
							<input class="selectRow" type="checkbox" name="ids" value="${dictWord.id}" />
						</td>
				        <td>${dictWord.name}</td>
				        <td>${dictWord.param}</td>
				        [#if !dict??]
				        	<td>${dict.name}</td>
				        [/#if]
				        <td>${dictWord.order}</td>
				        <td>${dictWord.createDate?string("yyyy-MM-dd HH:mm:ss")}</td>
				        <td class="center">
							<a href="${indexUrl}/${dictWord.id}/edit">
		...
		<div class="hide">
		    ...
		    <div class="dataTableActions">
		    	[#if dict??]
					<a href="${baseUrl}/dict" class="btn btn-default btn-sm">
						<i class="icon-mail-reply-all"></i>
						返回词典
					</a>
				[/#if]
		        <div class="btn-group">
					<a href="${indexUrl}/new[#if dict??]?dictId=${dict.id}[/#if]" class="btn btn-default btn-sm">
						<i class="icon-plus"></i>
						添加
					</a>
					<a href="${indexUrl}[#if dict??]?dictId=${dict.id}[/#if]" class="btn btn-default btn-sm">
