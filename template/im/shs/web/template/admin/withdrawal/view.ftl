...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>查看提现[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
						...
						<li class="dropdown">
							...
							<ul class="dropdown-menu">
								[#if withdrawal.state == "auditing"]
									<li>
						        		<a href="${indexUrl}_modif/${withdrawal.id}?redirectType=view[#if state??]&state=${state}[/#if]">修改</a>
									</li>
									<li>
										<a href="${indexUrl}_audit/${withdrawal.id}?redirectType=view[#if state??]&state=${state}[/#if]">审核</a>
									</li>
				            	[/#if]
								[#if withdrawal.state == "transferring"]
									<li>
										<a href="${indexUrl}_transfer/${withdrawal.id}?redirectType=view[#if state??]&state=${state}[/#if]">转账</a>
									</li>
				            	[/#if]
				            	[#if withdrawal.state == "auditing" || withdrawal.state == "transferring"]
				            		<li>
										<a href="${indexUrl}_cancel/${withdrawal.id}?redirectType=view[#if state??]&state=${state}[/#if]">取消</a>
									</li>
				            	[/#if]
				            	[#if withdrawal.state == "cancel" || withdrawal.state == "failure"]
				            		<li>
										<a href="${indexUrl}_remedy/${withdrawal.id}?redirectType=view[#if state??]&state=${state}[/#if]">补救</a>
									</li>
				            	[/#if]
							</ul>
						</li>
