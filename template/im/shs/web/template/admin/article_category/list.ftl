...
<html lang="en">
	<head>
		...
		[#-- 标题 --]
		<title>文章分类列表[#if systemPowered] - Powered By ICLNetwork[/#if]</title>
		[#-- Link 顶部 --]
		...
		<table class="table table-striped table-bordered dataTable">
			...
			<tbody>
				[#list list as articleCategory]
				    <tr>
				        <td>
				        	<a href="${indexUrl}?parentId=${articleCategory.id}" class="hint--top hint--info" data-hint="查看下级文章分类">${articleCategory}</a>
			        	</td>
			        	<td>${articleCategory.alias}</td>
				        <td>${articleCategory.builtin?string("是", "否")}</td>
				        <td>
							[#if articleCategory.description??]
								${abbreviate(articleCategory.description, 50, "...")}
							[#else]
								-
							[/#if]
				        </td>
